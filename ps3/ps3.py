...
def substitute_hand(hand, letter):
    ...
    """
    new_hand = hand
    taken = list(new_hand.keys())
    count = new_hand.pop(letter, 0)  # remove requested letter
    if count == 0:  # letter was not in hand
        return hand
    letters = string.ascii_lowercase
    available = [letter for letter in letters if letter not in taken]
    ...
    * Returns the total score for the series of hands
    ...
    while hand_count > 0:
        if not substituted:
            print('Current Hand: ', end='')
            display_hand(hand)
            confirm = input('Would you like to substitute a letter? ')
        ...
        if not replayed:
            confirm = input('Would you like to replay the hand? ')
            if confirm.lower() == 'yes':
                replayed = True
